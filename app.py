...
def verify_recaptcha(recaptcha_token):
    ...
    try:
        secret_key = SECRET_KEY  # Google's test secret key
        ...
        # For testing purposes, allow scans to proceed even if reCAPTCHA fails
        print("Allowing scan to proceed despite reCAPTCHA error (for testing)")
        return False
...
@app.route('/finial')
def finial():
    return render_template('finial.html', site_key=SITE_KEY)
...
def scan():
    try:
        ...
        # Verify reCAPTCHA if token is provided (required for finial.html, optional for index.html quick scan)
        if recaptcha_token:
           if not verify_recaptcha(recaptcha_token):
              print(f"reCAPTCHA verification failed for token: {recaptcha_token[:20]}...")
